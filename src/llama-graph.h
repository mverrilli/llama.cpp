...
public:
    ...
    // KPC int4 K-cache: per-token seq + position, drives the (seq,group) pool assignment
...
public:
    ...
    // KPC int4 K-cache: per-token seq + position (base and SWA sub-caches)
    ggml_tensor * self_kpc_seq     = nullptr; // I32 [n_batch]
    ggml_tensor * self_kpc_pos     = nullptr; // I32 [n_batch]
...
struct llm_graph_context {
    ...
    ggml_tensor * build_attn_mha(
            ...
            ggml_tensor * v_mla,   // [n_embd_head_v_mla, n_embd_head_v, n_head_v]
                  float   kq_scale,
                    int   il,
            ggml_tensor * k_scalezp  = nullptr,   // KPC: per-channel scale/zp
