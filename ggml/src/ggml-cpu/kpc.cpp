...
// inverse of ggml_kpc_deq: quantize v at scale s, zero-point z into channel c's nibble.
// round-half-up + [0,15] clamp must match every K write/requant site (s != 0 enforced upstream).
...
// fused q4_0-V dequant+mad: VKQ += dequant(q4_0 block)*vs (no V32 buffer / to_float).
// block = {f16 d, u8 qs[16]} (18 B); symmetric: deq = (q-8)*d; lo nibbles -> [0,16), hi -> [16,32).
KPC_TGT_AVX2
static void kpc_vmad_q4_0_avx2(float * VKQ, const uint8_t * vb, float vs, int64_t DV) {
    const __m128i mlo = _mm_set1_epi8(0x0F);
    const __m256  v8  = _mm256_set1_ps(8.0f);
    for (int64_t b = 0; b < DV/32; ++b) {
        const uint8_t * blk = vb + b*18;
        const __m256 vd  = _mm256_set1_ps(GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *)(blk + 0)));
        const __m256 vvs = _mm256_set1_ps(vs);
        const __m128i bytes = _mm_loadu_si128((const __m128i *)(blk + 2));
        const __m128i nib[2] = { _mm_and_si128(bytes, mlo), _mm_and_si128(_mm_srli_epi16(bytes, 4), mlo) };
        float * out = VKQ + b*32;
        for (int h = 0; h < 2; ++h) {                       // lo/hi -> slot ranges
            const __m256 f0 = _mm256_cvtepi32_ps(_mm256_cvtepu8_epi32(nib[h]));
            const __m256 f1 = _mm256_cvtepi32_ps(_mm256_cvtepu8_epi32(_mm_srli_si128(nib[h], 8)));
            float * o = out + h*16;
            // deq = (q-8)*d (exact, matches to_float), then VKQ += deq*vs
            _mm256_storeu_ps(o + 0, _mm256_fmadd_ps(_mm256_mul_ps(_mm256_sub_ps(f0, v8), vd), vvs, _mm256_loadu_ps(o + 0)));
            _mm256_storeu_ps(o + 8, _mm256_fmadd_ps(_mm256_mul_ps(_mm256_sub_ps(f1, v8), vd), vvs, _mm256_loadu_ps(o + 8)));
        }
    }
}
...
static void kpc_prescale_avx2(const float * pq, const float * sc, const float * zp,
    ...
    // corr summed sequentially to stay bit-identical; SIMD reorder/FMA would shift the sum
    float corr = 0.0f;
...
static int32_t kpc_dot_i8_neon(const int8_t * qs8, const uint8_t * krow, int64_t DK) {
    // dual accumulators break the loop-carried dependency so the NEON pipes overlap
    ...
    for (int64_t d = 0; d < DK; d += 32) {
        ...
        // nibbles are 0..15, identical under signed/unsigned i8, so SDOT is a bit-exact drop-in
        // for the vmull/vmlal/vpadal fallback below.
...
// VKQ += dequant(q4_0 block)*vs; block = {f16 d, u8 qs[16]} (18 B); deq = (q-8)*d; lo -> [0,16), hi -> [16,32).
static void kpc_vmad_q4_0_neon(float * VKQ, const uint8_t * vb, float vs, int64_t DV) {
    const float32x4_t vvs = vdupq_n_f32(vs);
    const float32x4_t v8  = vdupq_n_f32(8.0f);
    for (int64_t b = 0; b < DV/32; ++b) {
        const uint8_t * blk = vb + b*18;
        const float32x4_t vd = vdupq_n_f32(GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *)(blk + 0)));
        const uint8x16_t bytes = vld1q_u8(blk + 2);
        const uint8x16_t nib[2] = { vandq_u8(bytes, vdupq_n_u8(0x0F)), vshrq_n_u8(bytes, 4) };
        float * out = VKQ + b*32;
        for (int h = 0; h < 2; ++h) {                       // h=0 lo -> [0,16), h=1 hi -> [16,32)
            const uint16x8_t w0 = vmovl_u8(vget_low_u8(nib[h]));
            ...
            float * o = out + h*16;
            for (int k = 0; k < 4; ++k)                      // deq = (q-8)*d, VKQ += deq*vs
                vst1q_f32(o + k*4, vmlaq_f32(vld1q_f32(o + k*4), vmulq_f32(vsubq_f32(f[k], v8), vd), vvs));
        }
    }
}
...
// VKQ += dequant(q4_0 V row)*vs, fused; DV multiple of 32
static inline void kpc_vmad_q4_0(float * VKQ, const uint8_t * vb, float vs, int64_t DV) {
#ifdef KPC_X86
    if ((DV & 31) == 0 && kpc_have_avx2()) { kpc_vmad_q4_0_avx2(VKQ, vb, vs, DV); return; }
#elif defined(KPC_ARM)
    if ((DV & 31) == 0) { kpc_vmad_q4_0_neon(VKQ, vb, vs, DV); return; }
#endif
    for (int64_t b = 0; b < DV/32; ++b) {
        const uint8_t * blk = vb + b*18;
        const float d = GGML_CPU_FP16_TO_FP32(*(const ggml_fp16_t *)(blk + 0));
        ...
        float * o = VKQ + b*32;
        for (int j = 0; j < 16; ++j) { o[j] += (((qs[j] & 0x0F) - 8)*d)*vs; o[j+16] += (((qs[j] >> 4) - 8)*d)*vs; }
    }
}
...
void ggml_compute_forward_kpc_flash_attn(const struct ggml_compute_params * params, struct ggml_tensor * dst) {
    ...
    const bool    vq40 = v->type == GGML_TYPE_Q4_0;
    // other quantized V (q8_0, ...): dequant the row via the type's to_float, then SIMD-mad.
    const ggml_to_float_t v_to_float = (vf16 || vf32 || vq41 || vq40) ? NULL : ggml_get_type_traits(v->type)->to_float;
    GGML_ASSERT(DV <= 512);
    GGML_ASSERT(DK <= 512);
    GGML_ASSERT(vf16 || vf32 || vq41 || vq40 || v_to_float);
    GGML_ASSERT(q->type == GGML_TYPE_F32 && q->nb[0] == sizeof(float) && "KPC fused attn: q must be contiguous f32");

    // sibling q-heads of a kv head share the K/V/mask/gidx walk, each with its own
    // qs8/dqg/corr + VKQ/S/M; gsz shrinks for load balance.
    ...
    for (int64_t ir = r0; ir < r1; ++ir) {
        ...
        while (ic < NKV) {
            ...
            for (; ic < NKV; ++ic) {
                ...
                if ((int64_t) gidx[ic] != gkey) {     // compare the raw key so ic_start always makes progress
                ...
                for (int64_t hi = 0; hi < hcnt; ++hi) {
                    ...
                    else if (vq40) kpc_vmad_q4_0(VKQ[hi], (const uint8_t *) vd, vs, DV);
                    else {
...
// in-place per-channel int4 K write. groups by logical position (pos/32, per token via kpc_seq/kpc_pos) and
// scatters each cell to the physical slot from k_idxs, so SWA/defrag-recycled slots work. each (seq,group)
// maps to its own scalezp pool, so each sequence's scales stay independent. for each (seq,group):
// members = this call's tokens ++ (if the seq's open group) its staged f16 originals (k_resid); per-channel
// scale/zp -> packed nibbles; scalezp[pool] and group_index[slot] = pool. only the highest group per seq
// stays open. writes dst/scalezp/k_resid/group_index and the staging state in place; work is split into
// (seq, pool) items so a pool slab and its cells are owned by one thread, with the staging update deferred
// past a barrier (phase 2) so the per-call staging reads stay race-free.
// op_params unused; n_tokens/kv_size/ng_max/n_seq_max come from the src tensor shapes.
void ggml_compute_forward_kpc_write(const struct ggml_compute_params * params, struct ggml_tensor * dst) {
    ...
    // every token's primary seq must fit the staging / pool-band sizing (the host enforces this too)
    if (ith == 0) {
        for (int64_t t = 0; t < n_tokens; ++t) {
            GGML_ASSERT(kpc_seq[t] >= 0 && kpc_seq[t] < n_seq_max && "KPC write: seq_id out of range");
        }
    }
    ...
    // pools this call (re-)encodes; one absent here is touched by no thread, so the relocate below can
    // claim it without racing an encode.
    ...
    int64_t slot_of[32];
    auto gather = [&](int64_t sb, int64_t lg) -> uint32_t {
        for (int64_t w = 0; w < G; ++w) { tok_of[w] = -1; slot_of[w] = -1; }
        uint32_t members = 0;
        for (int64_t t = 0; t < n_tokens; ++t) {
            if ((int64_t) kpc_seq[t] != sb) continue;
        ...
        if (lg == sgp[sb]) {
            ...
            for (int64_t w = 0; w < G; ++w) {
                if ((prev_mask & (1u << w)) && !(members & (1u << w))) {
                    members  |= (1u << w);
                    slot_of[w] = rsl[sb*G + w];   // staged slot
                }
            }
        }
        return members;
    ...
    for (size_t wi = 0; wi < work.size(); ++wi) {
        ...
        const int64_t rbase = sb * (C*G);                    // k_resid per-sequence slab (f16 elems)
        ...
        const uint32_t members = gather(sb, lg);
        if (members == 0) {
...

        {
            ...
            // live cells still mapped to this pool but not in this write get dequantized against the
            // current slab and repacked with the new scale, so a re-encode never orphans them. gid < 0 = free.
            resc_slots.clear();
            resc_vals.clear();
            ...
            // relocate live non-member cells (e.g. seq_cp-shared) to a free pool instead of combining
            // them into this one: copy their scale slab and repoint gid, leaving the int4 rows untouched
            // (no requant). A free pool is one no work item touches and no live cell references, so the
            // move can't race another thread. Falls back to the combine below when the band is full.
            if (!resc_slots.empty()) {
                int64_t fp = -1;
                for (int64_t cand = 0; cand < ng_max; ++cand) {
                    if ((cand % nth) != ith) continue;   // disjoint per-thread candidates: no two claim the same pool
                    if (cand == pool || work_pools[cand]) continue;
                    bool used = false;
                    for (int64_t p = 0; p < kv_size; ++p) {
                ...
                if (fp >= 0) {
                    memcpy(szd + sbase + fp*sz->nb[1], szg, (size_t) sz->nb[1]);   // copy scale slab verbatim
    ...
    // phase 2: staging update. only the highest group of a seq stays open; re-stage its members
    ...
    for (size_t wi = 0; wi < work.size(); ++wi) {
        ...
        const int64_t rbase  = sb * (C*G);
        const int64_t slbase = sb * G;
